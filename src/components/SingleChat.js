...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      socket.emit("join chat", { room: selectedChat, name: user.name });
    } catch (error) {
      toast({
        title: "Error Occured!",
  ...
  const sendMessage = async (event) => {
    if (event.key === "Enter" && newMessage) {
      ...
      try {
        ...
        setNewChat({ ...selectedChat, latestMessage: data });
      } catch (error) {
        toast({
          title: "Error Occured!",
  ...
  useEffect(() => {
    socket.on("changeName", (room) => {
      setNewChat(room);
    });
  }, []);

  useEffect(() => {
    const newChats = chats?.filter((c) => c._id !== newChat?._id);
    newChats && setChats([newChat, ...newChats]);
  }, [newChat]);
